...
function agregarCarrito(parametro){
    var elemento = {
        nomproducto: productos[parametro].nomproducto,
...
function registrarCompra(){
    ...
    for(let i = 0; i < carrito.length; i++){
        for(let j = 0; j < losProductos.length; j++){
                if(losProductos[j].nomproducto == carrito[i].nomproducto){
...
function eliminarCompra(posicion){
    ...
    Swal.fire({
        title: "Está seguro?",
        text: "La compra se eliminara!!! y se descontará del stock",
        icon: "warning",
        ...
        if (result.isConfirmed) {
            ...
            //actualizamos el stock de productos
                for(let i = 0; i < laCompra.detalles.length; i++){
                        for(let j = 0; j < losProductos.length; j++){
                            if(losProductos[j].nomproducto == laCompra.detalles[i].nomproducto){
...
function verCompra(posicion){
   //var vercomprita = compras[posicion];
   //console.log(vercomprita);
...
function mostrarCompra(){
    ...
    //localStorage.setItem('laCompra', JSON.stringify(laCompra));
    //window.location.href = 'comprasVer.html';
}
